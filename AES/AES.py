...
#Handling input/output
    def takeIn(IN):
        AES.fileIN = open(f"C:\\Users\\Ryan Krasinski\\2122\\SRIP\\AES\\{IN}.txt", "r")
    ...
    def encrypt(text):
        ...
        for i in range(16):
            if i == 0:
                AES.keyGen(AES.linesIN, i)
                final = []
                for j in range(4):
                    temp = []
                    for k in range(4):
                        temp.append(AES.key[j][k]^AES.linesIN[j][k])
                    final.append(temp)
                AES.linesIN = final
            else:
                final = []
                for j in range(4):
                    temp = []
                    for k in range(4):
                        temp.append(int(AES.subBytes(hex(AES.linesIN[j][k]))))
                    final.append(temp)
                AES.linesIN = final
                AES.linesIN = AES.shiftRows(AES.linesIN)
                temp = []
                for l in AES.linesIN:
                    temp.append(AES.matMult(l))
                AES.linesIN = temp
                AES.keyGen(AES.linesIN, i)
                final = []
                for j in range(4):
        ...
        for j in range(4):
            for k in range(4):
                final.append(hex(AES.linesIN[j][k]) + " ")
        AES.linesOUT = final
